...
class WebsiteBookingController(http.Controller):
    ...
    def book_calendar_create(self, **kw):
        ...
        calendar = request.env['website.calendar'].sudo().browse(
            int(values['calendar_id']))

        # worlds worse way of converting to UTC...
        parse_string = values['start']
        if "+" in parse_string:
            start_date = parse(parse_string.replace(
                "+", "-")).astimezone(pytz.utc)
        ...
        cal_event = request.env['calendar.event'].sudo().create({'user_id': calendar.user_id.id, 'name': values[
            'name'] + " (Booking)", 'start': start_date, 'stop': stop_time, 'booking_email': values['email'], 'description': values['email'] + "\n" + values['comment']})
    ...
    def book_calendar(self, calendar, **kw):
        ...
        my_calendar = request.env[
            'website.calendar'].sudo().browse(int(calendar))
        return request.render('ob_website_calender_booking.website_calendar_booking_page', {'calendar': my_calendar})
    ...
    def book_calendar_timeframe(self, calendar, **kw):
        ...
        for time_frame in my_calendar.time_frame_ids:
            ...
            return_string += '"end": "' + \
                str(time_frame.end_time).replace(".", ":") + '",'
    ...
    def book_calendar_events(self, user, **kw):
        ...
        for event in request.env['calendar.event'].sudo().search([('user_id', '=', int(user))]):
            ...
            return_string += '"start": "' + \
                str(event.start_datetime) + '+00:00",'
