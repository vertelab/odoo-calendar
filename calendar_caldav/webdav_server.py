...
class DAVHandler(HttpOptions, DAVRequestHandler, FixSendError):
    ...
    def setup(self):
        self.davpath = '/x'+config.get_misc('webdav','vdir','webdav')
...
try:
    ...
    if (config.get_misc('webdav','enable',True)):
        directory = '/x'+config.get_misc('webdav','vdir','webdav')
        handler = DAVHandler
        ...
        if not (config.get_misc('webdav', 'no_root_hack', False)):
            ...
            _logger.info("WebDAV Static service registered at path: %s/ "% directory)
except Exception, e:
